 ...
 *
 * During serialization, pointers are stored as offsets from the start of the
 * serialized data.  This function converts those offsets back to proper
 * pointers: isnull/values array offsets and non-byval Datum offsets are all
 * fixed up to point to the correct locations in the deserialized structure.
 */
JoinMCVList *
					   ...
					   VacAttrStats **vacattrstats)
{
	...
	/*
	 * Extract join keys TODO: currently only supports single join pair
	 */
	...
	/*
	 * We reuse the already-computed MCV statistics for the FK column. This
	 * gives us ~100 most common values with their frequencies already
	 * calculated by the current ANALYZE.
	 *
	...
	/*
	 * For each MCV value of the FK column, look up the corresponding filter
					  ...
					  int2vector *filter_attnums)
{
	...
	while (HeapTupleIsValid(htup = systable_getnext(scan)))
	{
		...
		if (filter_attnums)
		{
			 ...
			 */
			for (i = 0; i < filter_attnums->dim1; i++)
			{
				bool		found = false;

				for (j = 0; j < stxkeys->dim1; j++)
				{
					if (filter_attnums->values[i] == stxkeys->values[j])
					{
						found = true;
						break;
					}
...
/*
 * find_join_mcv_pattern
 *		Detect if a join+filter combination matches a join MCV pattern.
 *
 ...
 *
 * Returns an allocated JoinStatsMatch if pattern is found, NULL otherwise.
 * The caller is responsible for freeing the returned structure.
 */
JoinStatsMatch *
find_join_mcv_pattern(PlannerInfo *root,
					  ...
					  List *restrictlist)
{
	...
	/* Build the JoinStatsMatch result */
	match = palloc(sizeof(JoinStatsMatch));
	...
	match->target_rel = target_rte->relid;
	match->target_joinkey = target_var->varattno;
	match->other_rel = filtered_rte->relid;
	match->other_joinkey = filtered_var->varattno;
	match->filter_attnums = filter_attnums_list;
	match->filter_values = list_length(filter_values_list) == 1 ?
		linitial(filter_values_list) : filter_values_list;
	match->collation = collation;

	/* Track which clauses were used */
	match->join_rinfos = list_make1(join_rinfo);
	match->filter_rinfos = filter_rinfos_list;

	return match;
}

/*
 * find_join_mcv_pattern_from_clauses
 ...
 * then calls find_join_mcv_pattern() with those relations.
 *
 * Returns NULL if no join mcv stats would possibly be applicable.
 */
JoinStatsMatch *
find_join_mcv_pattern_from_clauses(PlannerInfo *root, List *clauses)
{
	...
	match = find_join_mcv_pattern(root, rel1, rel2, clauses);
	if (match)
		return match;

	return find_join_mcv_pattern(root, rel2, rel1, clauses);
}
